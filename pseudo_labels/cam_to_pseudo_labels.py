...
def _work(process_id, infer_dataset, args):
    ...
    for iter, pack in enumerate(infer_data_loader):
        ...
        img = pack['image'][0].numpy()
        cam_path = os.path.join(args.cam_out_dir, img_name + '.npy')
        ...
        conf = np.zeros_like(cams) + 255
        conf[cams > 0.9] = 1
        conf[cams < 0.3] = 0

        imageio.imwrite(os.path.join(args.pseudo_labels_out_dir, img_name + '.png'), np.squeeze(conf.astype(np.uint8)))
        saved += 1
